...
def get_current_git_head() -> str:
    try:
        ...
        raise QualificationError("BLOCKED_ENVIRONMENT", f"Failed to determine git rev-parse HEAD: {e}")
...
def run_local_qualification(cache_dir: Path) -> dict:
    current_head = get_current_git_head()
    if current_head != REQUIRED_FOUNDRY_BASE_SHA:
        return {
            "terminal_state": "STALE_TARGET",
            "required_foundry_base_commit": REQUIRED_FOUNDRY_BASE_SHA,
            "actual_foundry_base_commit": current_head,
            "message": f"Foundry main HEAD ({current_head}) does not match required base commit ({REQUIRED_FOUNDRY_BASE_SHA}).",
        }
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, help="Path to write evidence record JSON")
    args = parser.parse_args()

    evidence = run_local_qualification(args.cache_dir)
