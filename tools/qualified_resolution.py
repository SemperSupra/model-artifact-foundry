...
def resolution_context(request_doc: Any, environment_digest: str, runtime_doc: Any) -> dict[str, Any]:
...
def resolution_key(context: Any) -> str:
    ...
    _keys(context, "resolution", {"request_digest", "target_profile_id", "environment_digest", "runtime"})
    normalized = {
...
def binding_filename(key: str) -> str:
    return _digest(key, "resolution_key").removeprefix("sha256:") + ".json"
...
def validate_binding(binding_doc: Any) -> dict[str, Any]:
    ...
    _keys(binding, "binding", {"schema_version", "resolution", "resolution_key", "qualification", "selection", "binding_digest"})
    ...
    _keys(context, "binding.resolution", {"request_digest", "target_profile_id", "environment_digest", "runtime"})
    context = {
        ...
        "runtime": _normalize_runtime_context(context["runtime"], context="binding.resolution.runtime"),
    }
    ...
    _keys(artifact, "binding.selection.artifact", {"provider", "repository", "source_revision"})
    artifact = {
...
def lookup(request_doc: Any, environment_digest: str, runtime_doc: Any, store: str | Path) -> dict[str, Any]:
    request = REQUEST.normalize_request(request_doc)
...
def main() -> int:
    ...
    if args.command == "retain":
        binding = binding_from_qualified_receipt(
            request_doc, args.environment_digest, runtime, _load(args.receipt)
        )
        print(retain_binding(args.store, binding))
        return 0
