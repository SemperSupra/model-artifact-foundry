...
def resolution_context(request_doc: Any, environment_digest: str) -> dict[str, str]:
...
def resolution_key(context: Any) -> str:
    ...
    _keys(context, "resolution", {"request_digest", "target_profile_id", "environment_digest"})
    normalized = {
...
def validate_binding(binding_doc: Any) -> dict[str, Any]:
    ...
    _keys(context, "binding.resolution", {"request_digest", "target_profile_id", "environment_digest"})
    context = {
...
def lookup(request_doc: Any, environment_digest: str, store: str | Path) -> dict[str, Any]:
    request = REQUEST.normalize_request(request_doc)
    context = resolution_context(request, environment_digest)
...
def main() -> int:
    ...
    result = lookup(request_doc, args.environment_digest, args.store)
    if args.output:
