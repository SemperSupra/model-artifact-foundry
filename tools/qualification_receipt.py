...
def validate_subject(subject: Any) -> dict[str, Any]:
    ...
    _require_keys(
        ...
        required={"provider", "repository", "source_revision", "identity_strength", "observation_digest"},
    )
    for key in ("provider", "repository", "source_revision", "identity_strength"):
        _require_text(artifact[key], f"subject.artifact.{key}")
    _require_digest(artifact["observation_digest"], "subject.artifact.observation_digest")
