...
def qualified_receipt(
    *,
    ...
    draft["subject"]["target"]["environment_digest"] = environment_digest or environment()
    if runtime_context is not None:
        draft["subject"]["runtime"] = copy.deepcopy(runtime_context)
    if recorded_at is not None:
        draft["recorded_at"] = recorded_at
    if observation_digest is not None or observation_ref is not None:
        item = next(e for e in draft["evidence"] if e["kind"] == "provider_observation")
        if observation_digest is not None:
            item["digest"] = observation_digest
        if observation_ref is not None:
            item["ref"] = observation_ref
...
def test_changed_request_target_environment_or_runtime_changes_resolution_key():
    ...
    base = RESOLUTION.resolution_key(RESOLUTION.resolution_context(base_request, environment(), base_runtime))
    ...
    for field, digest in (
        ...
        changed_runtime = copy.deepcopy(base_runtime)
        changed_runtime[field] = digest
        assert RESOLUTION.resolution_key(
            RESOLUTION.resolution_context(base_request, environment(), changed_runtime)
        ) != base
...
def test_binding_is_derived_from_qualified_receipt_and_uses_stable_selection():
    receipt = qualified_receipt()
    binding = RESOLUTION.binding_from_qualified_receipt(request(), environment(), runtime(), receipt)
    ...
    assert binding["selection"]["artifact"] == {
        "provider": receipt["subject"]["artifact"]["provider"],
        "repository": receipt["subject"]["artifact"]["repository"],
        "source_revision": receipt["subject"]["artifact"]["source_revision"],
    }
    assert "observation_digest" not in binding["selection"]["artifact"]
...
def test_provider_observation_recapture_does_not_change_subject_selection_or_resolution():
    first = qualified_receipt()
    second = qualified_receipt(
        observation_digest="sha256:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",
        observation_ref="public://foundry/observations/kv-ground-recaptured.json",
    )
    assert first["subject_key"] == second["subject_key"]
    assert first["record_digest"] != second["record_digest"]

    first_binding = RESOLUTION.binding_from_qualified_receipt(request(), environment(), runtime(), first)
    second_binding = RESOLUTION.binding_from_qualified_receipt(request(), environment(), runtime(), second)
    assert first_binding["resolution_key"] == second_binding["resolution_key"]
    assert first_binding["selection"] == second_binding["selection"]
...
def test_rejected_receipt_cannot_create_known_resolution():
    with pytest.raises(RESOLUTION.QualifiedResolutionError, match="only a qualified receipt"):
        RESOLUTION.binding_from_qualified_receipt(request(), environment(), runtime(), rejected_receipt())
...
def test_receipt_request_environment_or_runtime_mismatch_is_rejected():
    ...
    with pytest.raises(RESOLUTION.QualifiedResolutionError, match="request digest"):
        RESOLUTION.binding_from_qualified_receipt(changed, environment(), runtime(), receipt)
    ...
    with pytest.raises(RESOLUTION.QualifiedResolutionError, match="runtime context"):
        RESOLUTION.binding_from_qualified_receipt(request(), environment(), changed_runtime, receipt)
...
def test_receipt_selection_must_satisfy_request_constraints():
    ...
    with pytest.raises(RESOLUTION.QualifiedResolutionError, match="does not satisfy request"):
        RESOLUTION.binding_from_qualified_receipt(req, environment(), runtime(), receipt)
